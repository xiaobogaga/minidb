...
const (
	...
	ER_HTON_CONTROL_CMD_NOT_IMPLEMENTED                                   = 999
	ER_HASHCHK                                                            = 1000
	ER_NISAMCHK                                                           = 1001
	ER_NO                                                                 = 1002
	ER_YES                                                                = 1003
...
func init() {
	ErrorCodeMsgMap[ER_ACCESS_DENIED_NO_PASSWORD_ERROR] = MysqlError{
		errorCode: ER_ACCESS_DENIED_NO_PASSWORD_ERROR,
		errorMsg:  "Access denied for user '%s'@'%s'",
	}
	ErrorCodeMsgMap[ER_ACCESS_DENIED_ERROR] = MysqlError{
		errorCode: ER_ACCESS_DENIED_ERROR,
		errorMsg:  "Access denied for user '%s'@'%s' (using password: %s)",
	}
	ErrorCodeMsgMap[ER_NET_READ_ERROR] = MysqlError{
		errorCode: ER_NET_READ_ERROR,
		errorMsg:  "Got an error reading communication packets",
	}
	ErrorCodeMsgMap[ER_NET_READ_INTERRUPTED] = MysqlError{
		errorCode: ER_NET_READ_INTERRUPTED,
		errorMsg:  "Got timeout reading communication packets",
	}
	ErrorCodeMsgMap[ER_NET_WRITE_INTERRUPTED] = MysqlError{
		errorCode: ER_NET_WRITE_INTERRUPTED,
		errorMsg:  "Got timeout writing communication packets",
	}
	ErrorCodeMsgMap[ER_NET_ERROR_ON_WRITE] = MysqlError{
		errorCode: ER_NET_ERROR_ON_WRITE,
		errorMsg:  "Got an error writing communication packets",
	}
	ErrorCodeMsgMap[ER_NET_PACKETS_OUT_OF_ORDER] = MysqlError{
		errorCode: ER_NET_PACKETS_OUT_OF_ORDER,
		errorMsg:  "Got packets out of order",
	}
	ErrorCodeMsgMap[ER_UNKNOWN_COM_ERROR] = MysqlError{
